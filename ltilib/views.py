...
def tool_config(request):
    ...
    lti_tool_config.set_ext_param(
        "canvas.instructure.com", "domain", request.get_host().partition(":")[0]
    )
...
def launch_resource_selection(request):
    ...
    if "ext_content_return_types" in request.POST:
        more_lti_params = {
            "ext_content_return_types": request.POST.get(
                "ext_content_return_types", None
            ),
            "ext_content_intended_use": request.POST.get(
                "ext_content_intended_use", None
            ),
    ...
    return HttpResponseRedirect(
        "{return_url}?return_type=lti_launch_url&url={launch_url}".format(
            ...
            launch_url=request.build_absolute_uri(
                reverse("lti:launch", kwargs={"quiz_id": quiz_id})
            ),
        )
    )
