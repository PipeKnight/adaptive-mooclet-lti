...
class QSF:
    ...
    def upload_to_qualtrics(self, survey_name, qsf_url, method=None):
        ...
        if not response.status_code == 200:
            raise Exception(
                "Something went wrong with qualtrics survey creation: {}".format(
                    response.text
                )
            )
...
def provision_qualtrics_quiz(request, question):
    ...
    survey_name = "adaptive-quiz question_{} june2 {}".format(
        question.id, question.text
    )

    # this is the url that the modified QSF will be hosted at
    qsf_url = request.build_absolute_uri(
        reverse("qualtrics:qsf_for_question", kwargs={"question_id": question.id})
    )
