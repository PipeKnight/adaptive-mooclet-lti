...
urlpatterns = [
    ### resource selection ###
    url(
        r"^quiz_create_options$", views.quiz_create_options, name="quiz_create_options"
    ),
    ...
    url(
        r"^(?P<quiz_id>[0-9]+)/question/(?P<question_id>[0-9]+)/answer/(?P<answer_id>[0-9]+)/",
        include("engine.urls.urls_answer"),
    ),
    url(
        r"^(?P<quiz_id>[0-9]+)/question/(?P<question_id>[0-9]+)/",
        include("engine.urls.urls_question"),
    ),
