...
class MoocletType(models.Model):
    ...
    version_content_type = models.ForeignKey(
        ContentType,
        null=True,
...
class Version(models.Model):
    ...
    mooclet = models.ForeignKey(Mooclet, null=True, on_delete=models.DO_NOTHING)
...
class Policy(models.Model):
    ...
    def run_simulation(self, context, iterations):
        ...
        probabilities = policy_probability_function(
            variables, context, iterations=iterations
        )
...
class Variable(models.Model):
    ...
    content_type = models.ForeignKey(
        ContentType, null=True, on_delete=models.DO_NOTHING
    )  # Lets you reference a table. e.g. if content_type = Version, it's associated with a Version. Can have multiple values associated with it.
    ...
    def get_data(self, context=None):
        ...
        if context:
            related_object = (
                self.object_name
            )  # str: 'course','user','mooclet', or 'version'
            ...
            if "mooclet" in context and related_object == "version":
                query["object_id__in"] = context["mooclet"].version_set.values_list(
                    "id", flat=True
                )
            else:
                query["object_id"] = context[
                    related_object
                ].id  # pk of related content object instance
...
class Value(models.Model):
    ...
    object_id = models.PositiveIntegerField(
        null=True
    )  # This can just be the integer primary key of the table specified in Variable as content_type
...
class Quiz(models.Model):
    ...
    mooclet_next_question = models.ForeignKey(
        Mooclet, null=True, blank=True, on_delete=models.DO_NOTHING
    )
...
class Answer(models.Model):
    ...
    mooclet_explanation = models.ForeignKey(
        Mooclet, blank=True, null=True, on_delete=models.DO_NOTHING
    )  # explanation mooclet
